...
def train(self, obs, acs, rews_list, next_obs, terminals):
    ...
    advantage_values = self.estimate_advantage(obs, rews_list, q_values, terminals)
...
def estimate_advantage(self, obs, rews_list, q_values, terminals):
    ...
    if self.nn_baseline:
            ...
            if self.gae_lambda is not None:
                ...
                adv_n = np.zeros(batch_size + 1)
                for i in reversed(range(batch_size)):
                    delta = rews[i] + (self.gamma * b_n[i + 1] * (1 - terminals[i]) - b_n[i])
                    adv_n[i] = delta + self.gamma * self.gae_lambda * adv_n[i + 1] * (1 - terminals[i])
                adv_n = adv_n[:-1]
            else:
                adv_n = q_values - b_n
